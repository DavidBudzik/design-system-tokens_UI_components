...
/**
 * Design system data - manually typed since we can't directly import TS
 * This mirrors the structure from src/data/designSystemData.ts
 */
...
/**
 * Generate tokens.json file from design system data
 * This creates the JSON file that the Figma plugin can fetch
 */
function generateTokensFile() {
  ...
  const outputPath = join(publicDir, 'tokens.json');
  ...
  try {
    ...
    console.log(`📊 Total tokens: ${tokensExport.meta.totalTokens}`);
    console.log(`📦 Total sections: ${tokensExport.sections.length}`);
    console.log(`⏰ Generated at: ${tokensExport.meta.generatedAt}`);
    console.log(`🔗 Source: ${tokensExport.meta.source}\n`);

    return true;
  } catch (error) {
    console.error('\n❌ Error generating tokens.json:', error);
    process.exit(1);
  }
}
